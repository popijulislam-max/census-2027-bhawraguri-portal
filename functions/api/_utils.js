...
export async function sign(payload,secret){const key=await crypto.subtle.importKey('raw',enc.encode(secret),{name:'HMAC',hash:'SHA-256'},false,['sign']);const out=await crypto.subtle.sign('HMAC',key,enc.encode(payload));return b64(String.fromCharCode(...new Uint8Array(out)))}
export async function makeSession(username,secret){const payload=b64(JSON.stringify({u:username,t:Date.now()}));return payload+'.'+await sign(payload,secret)}
